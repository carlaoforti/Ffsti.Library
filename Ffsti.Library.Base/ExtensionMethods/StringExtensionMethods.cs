...
public static class StringExtensionMethods
{
    ...
    public static string Phonetized(this string text, bool keepVowels = true)
    {
        ...
        for (var i = 1; i <= sb.Length - 1; i += 1)
            if (result[result.Length - 1] != sb[i] || char.IsDigit(sb[i]))
                result.Append(sb[i]);
    ...
    public static string FormatBrazilianDocument(this string text)
    {
        if (string.IsNullOrEmpty(text)) return string.Empty;
        text = text.RemovePunctuation();
        return text.Length < 14 ? FormatCpf(text) : FormatCnpj(text);
    }
    ...
    public static bool ValidateBrazilianDocument(this string document)
    {
        ...
        {
            case 11:
                return document.ValidateCpf();
            case 14:
                return document.ValidateCnpj();
        }
        return false;
    }
    #endregion
    ...
    {
        return text?.Trim().ToUpper() ?? string.Empty;
    }

    //TODO: rever como implementar
